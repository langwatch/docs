...
2. **Code-First Experiments** - Define the entire experiment in code and run it in CI/CD
...
| **Code-First Experiments** | Version control your experiment config; full flexibility in code |

---

## Option 1: Platform Experiments

Configure your experiment once in the LangWatch Experiments Workbench, then trigger it from CI/CD.

### Setup

1. **Create your experiment** in the [Experiments Workbench](https://app.langwatch.ai/@project/evaluations)
   - Add your dataset
...
result = langwatch.evaluation.evaluate("your-experiment-slug")
...
const result = await langwatch.evaluation.run("your-experiment-slug");
result.printSummary();
```
  </Tab>
</Tabs>
...
## Option 2: Code-First Experiments
...
experiment = langwatch.evaluation.init("ci-quality-check")
...
// Initialize experiment
const experiment = await langwatch.evaluation.init("ci-quality-check");
...
Code-first experiments shine when comparing different configurations:

```python
import langwatch

dataset = langwatch.dataset.get_dataset("qa-dataset").to_pandas()
experiment = langwatch.evaluation.init("model-comparison-ci")
...
try:
    result = langwatch.evaluation.evaluate("my-experiment", timeout=300)
    result.print_summary()
...
try {
  const result = await langwatch.evaluation.run("my-experiment", { timeout: 300000 });
  result.printSummary();
} catch (error) {
...
<CardGroup cols={2}>
  <Card title="Experiments Workbench" icon="window" href="/evaluations/experiments/workbench/answer-correctness">
    Create experiments in the platform UI
  </Card>
  <Card title="Code-First Experiments" icon="code" href="/evaluations/experiments/code-first">
    Full guide to code-first experiments
  </Card>
