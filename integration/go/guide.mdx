...
### 1. Get your LangWatch API Key

First, you need a LangWatch API key. Sign up at [app.langwatch.ai](https://app.langwatch.ai) and find your API key in your project settings. Set it as an environment variable:
...
You need the core SDK, the OpenAI instrumentation middleware, and the OpenTelemetry exporter.
...
Here's a complete example that shows how to set up tracing and instrument an OpenAI client.
...
// setupLangWatch configures OpenTelemetry for LangWatch.
...
The OpenAI instrumentation is compatible with any provider that adheres to the OpenAI API spec. This includes Anthropic, Azure OpenAI, Groq, and local models via Ollama.

See the dedicated guides for more details:
- [OpenAI Integration](/integration/go/integrations/open-ai)
- [Other Provider Integrations](/integration/go/integrations/other)
...
| `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` | A standard OpenTelemetry variable that can also be used to set the endpoint URL. If set, it overrides `LANGWATCH_ENDPOINT`. |
...
LangWatch offers integrations with popular Go libraries and OpenAI-compatible providers:

- [OpenAI](/integration/go/integrations/open-ai) - Official OpenAI Go client
- [Other Providers](/integration/go/integrations/other) - Anthropic, Azure, local models, and more
...
- Explore [RAG implementation](/integration/go/tutorials/capturing-rag) with context tracking
- Set up [conversation threads](/integration/go/tutorials/capturing-threads) for multi-turn chats
