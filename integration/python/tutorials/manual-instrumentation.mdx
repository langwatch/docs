...
from langwatch.attributes import AttributeName # For semantic attribute keys
...
async def rag_retrieval_manual(query: str):
    ...
    async with langwatch.span(type="rag", name="RAG Document Retrieval") as span:
        ...
        span.update(
            ...
            metadata={"strategy": "manual_vector_search"},
...
async def handle_user_query_manual(query: str):
    ...
    async with langwatch.trace(name="Manual User Query Handling") as trace:
        trace.set_metadata({"user_id": "manual-user", "query": query}) # Set initial trace metadata
        ...
        llm_response = ""
        async with langwatch.span(name="Manual LLM Generation") as llm_span:
            llm_span.set_attribute(AttributeName.LANGWATCH_SPAN_TYPE, "llm") # Set span type
...
- Use methods like `trace.set_metadata()`, `span.add_event()`, and primarily `span.update(...)` / `trace.update(...)` to add details. The `update()` method is flexible for adding structured data like `input`, `output`, `metadata`, and `contexts`.
