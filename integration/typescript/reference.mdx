...
const promptVersion = await langwatch.prompts.getVersion("prompt-id", "version-id");

// Get a prompt version with variables (returns compiled prompt)
const compiledPromptVersion = await langwatch.prompts.getVersion("prompt-id", "version-id", {
  user_name: "John",
  product_name: "LangWatch"
});
```
...
const compiledPrompt = await langwatch.prompts.get("customer-support", {
