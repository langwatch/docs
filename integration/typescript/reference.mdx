...
// Create a prompt with advanced configuration
const prompt = await langwatch.prompts.create({
  handle: "sentiment-analyzer",
  scope: "PROJECT",
...
Retrieves a prompt from the LangWatch platform by handle.
...
const compiledPrompt = await langwatch.prompts.get("customer-support-bot", {
  user_name: "John",
...
#### `langwatch.prompts.getVersion()`

Retrieves a specific version of a prompt from the LangWatch platform.

```typescript
// Get a prompt version without variables
const promptVersion = await langwatch.prompts.getVersion("customer-support-bot", "version-id");

// Get a prompt version with variables (returns compiled prompt)
const compiledPromptVersion = await langwatch.prompts.getVersion("customer-support-bot", "version-id", {
  user_name: "John",
  product_name: "LangWatch"
});
```
...
<ParamField path="updates" type="UpdatePromptOptions" required>
  The fields to update on the prompt.
</ParamField>
...
```typescript
await langwatch.prompts.delete("customer-support-bot");
```
...
The LangWatch CLI provides dependency management for AI prompts as plain YAML files, enabling you to version prompts locally with Git while synchronizing with the LangWatch platform for testing, evaluation, and team collaboration.
...
<ResponseField name="scope" type="string" required>
  Access scope: "PROJECT" or "ORGANIZATION".
</ResponseField>
...
<ResponseField name="promptingTechnique" type="string">
  The prompting strategy: "few_shot", "in_context", or "chain_of_thought".
</ResponseField>
...
Template variables for prompt compilation. Prompts use `{{ variable_name }}` syntax for dynamic content.
...
const variables = {
  ...
  is_premium: true,
  items: ["item1", "item2"],
  user_data: { age: 30, role: "admin" }
};

// Usage in prompt template
const prompt = "Hello {{user_name}}! You have {{count}} items. Premium status: {{is_premium}}";
```
...
<ResponseField name="handle" type="string" required>
  Human-readable identifier for the prompt.
</ResponseField>
...
<ResponseField name="temperature" type="Optional<number>">
  Fine-tune creativity vs. consistency (0.0 = deterministic, 2.0 = very creative).
</ResponseField>

<ResponseField name="maxTokens" type="Optional<number>">
  Control response length and costs.
</ResponseField>

<ResponseField name="messages" type="Optional<Array>">
  Array of chat messages with roles and content (alternative to prompt field).
</ResponseField>

<ResponseField name="inputs" type="Optional<Array>">
  Array of input variable definitions with identifiers and types.
</ResponseField>

<ResponseField name="outputs" type="Optional<Array>">
  Array of output variable definitions with identifiers and types.
</ResponseField>

<ResponseField name="demonstrations" type="Optional<object>">
  Few-shot examples with columns and rows structure.
</ResponseField>

<ResponseField name="promptingTechnique" type="Optional<string>">
  The prompting strategy: "few_shot", "in_context", or "chain_of_thought".
</ResponseField>

### `UpdatePromptOptions`

Configuration options for updating an existing prompt.

<ResponseField name="prompt" type="Optional<string>">
  The main prompt text (system message).
</ResponseField>
...
<ResponseField name="inputs" type="Optional<Array>">
  Array of input variable definitions.
</ResponseField>

<ResponseField name="outputs" type="Optional<Array>">
  Array of output variable definitions.
</ResponseField>
