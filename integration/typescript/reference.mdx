...
Initializes the LangWatch client for Node.js environments, enabling data collection and tracing for your LLM application. This is typically the first function you'll call when integrating LangWatch.
...
<ParamField path="otelSpanProcessors" type="Optional<SpanProcessor[]>" default="[]">
  Additional OpenTelemetry span processors to add to the SDK. These will be added after the LangWatch SDK has been initialized.
</ParamField>
...
<ParamField path="disableOpenTelemetryAutomaticSetup" type="Optional<boolean>" default="false">
  If `true`, disables the automatic setup of the OpenTelemetry SDK. You will need to setup the OpenTelemetry SDK yourself, and ensure that a SpanProcessor is added that will send traces to the LangWatch API.
</ParamField>
...
For browser environments, use the browser-specific setup:

```typescript
import { setup } from "langwatch/browser";
...
The browser setup accepts the same options as the Node.js setup, except for `otelSpanProcessors` which are not supported in browser environments.
...
```typescript
import { getPrompt, getPromptVersion } from "langwatch";

// Get the latest version of a prompt
const compiledPrompt = await getPrompt("customer-support", {
  customer_name: "John Doe",
  issue_type: "billing"
});

// Get a specific version of a prompt
const specificVersion = await getPromptVersion("customer-support", "v1.2.3", {
  customer_name: "John Doe",
  issue_type: "billing"
});

console.log(compiledPrompt.content);
console.log(specificVersion.content);
```

### Prompt Selection in Tracing

```typescript
import { getLangWatchTracer } from "langwatch";
import { getPrompt } from "langwatch";

const tracer = getLangWatchTracer("my-service");

await tracer.withActiveSpan("llm-call", async (span) => {
  // Get a prompt and set it as selected for this span
  const prompt = await getPrompt("customer-support");
  span.setSelectedPrompt(prompt);
