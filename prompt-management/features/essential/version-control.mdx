...
curl --request GET \
  --url "https://app.langwatch.ai/api/prompts/{prompt_id}/versions" \
  --header "X-Auth-Token: your-api-key"

# Get a specific prompt (latest version)
curl --request GET \
  --url "https://app.langwatch.ai/api/prompts/{prompt_id}" \
  --header "X-Auth-Token: your-api-key"
...
curl --request GET \
  --url "https://app.langwatch.ai/api/prompts/{prompt_id}?version=2" \
  --header "X-Auth-Token: your-api-key"
```
...
<Tabs>
  <Tab title="TypeScript SDK">
    ...
    console.log(`Updated prompt: ${updatedPrompt.name}, New version: ${updatedPrompt.version}`);
    ```
  ...
  <Tab title="Python SDK">
    ...
    print(f"Updated prompt: {updated_prompt.name}, New version: {updated_prompt.version}")
    ```
...
<Tabs>
  <Tab title="TypeScript SDK">
    ...
    // Delete by ID (removes all versions)
    const resultById = await langwatch.prompts.delete("prompt_TrYXZLsiTJkn9N6PiZiae");
  ...
  <Tab title="Python SDK">
    ...
    # Delete by ID (removes all versions)
    result_by_id = langwatch.prompts.delete("prompt_TrYXZLsiTJkn9N6PiZiae")
