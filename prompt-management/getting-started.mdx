...
<Tabs>
  <Tab title="Python SDK">
    ...
    prompt = langwatch.prompts.get("customer-support-bot")  # by handle
    # or
    # prompt = langwatch.prompts.get("prompt_TrYXZLsiTJkn9N6PiZiae")  # by ID
  ...
  <Tab title="TypeScript SDK">
    ...
    // Get the latest prompt (can use handle or ID)
    const prompt = await langwatch.prompts.get("customer-support-bot"); // by handle
    // or
    // const prompt = await langwatch.prompts.get("prompt_TrYXZLsiTJkn9N6PiZiae"); // by ID
